...
class Square():
    ...
    """
    def __init__(self, model, norm='Linf', eps=16/255, n_queries=1000, n_restarts=1,
    ...
    def forward(self, images, labels):
        ...
        b, c, h, w = delta.shape
        Freq = torch.fft.fft(torch.fft.fft(delta.view(b, c, h*w).permute(0,2,1), dim=-1), dim=-2)

        b, patches, c = Freq.shape
        lowpass_filter = torch.zeros((1,patches,1)).cuda()
        lowpass_filter[:,int(patches * 0.4)+1 : int(patches * 0.6)] = 1
        # lowpass_filter[:,int(patches * 0.6) : int(patches * 0.7)] = 1
        # lowpass_filter[patches // 2] = 1
        # lowpass_filter[-int(patches*0.1):] = 1
        # print(lowpass_filter[0])
        
        low_Freq = Freq * lowpass_filter
        delta = torch.fft.ifft(torch.fft.ifft(low_Freq, dim=-2), dim=-1).real
        delta = delta.permute(0,2,1).view(b, c, h, w)

        return torch.clamp(images + delta, min=0, max=1)
